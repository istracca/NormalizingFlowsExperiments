...
def compute_centroids(model, loader, num_classes, device):
    """
    Computes Latent Centroids for SingleMNIST using the training dataset.
    """
    model.eval()
    all_z = []
    all_y = []
    ...
    centroids = torch.zeros((num_classes, latent_dim))
    for c in range(num_classes):
        mask = (all_y == c)
        if mask.sum() > 0:
            centroids[c] = all_z[mask].mean(dim=0)
        else:
            centroids[c] = torch.full((latent_dim,), float('inf'))
            
    return centroids.to(device)

def evaluate_model(model, prior, loader, device, n_runs, centroids):
    """Computes Loss and Classification based on Z-distance to empirical centroids."""
    model.eval()
    prior.eval()
    ...
    for _ in range(n_runs):
        ...
        with torch.no_grad():
            for batch_X, batch_y in loader:
                ...
                dists = torch.cdist(z_flat, centroids)
...
X_train_t = torch.tensor(data['X_train'].reshape(-1, 1, 28, 28), dtype=torch.float32)
...
for config in combinations:
    ...
    original_model_path = f"../experiments/models/Gaussian/{config_id}.pth"
    if os.path.exists(original_model_path):
        all_model_paths.append(original_model_path)
    
    search_pattern = f"../experiments_seed/models/Gaussian/{config_id.rstrip('_')}_*.pth"
    ...
    for model_path in all_model_paths:
        ...
        v_acc_all, v_losses = evaluate_model(model, prior, val_loader, DEVICE, EVAL_RUNS_PER_SEED, centroids)
        seed_val_accs.append(np.mean(v_acc_all))
        seed_val_losses.append(np.mean(v_losses))

        t_acc_all, t_losses = evaluate_model(model, prior, test_loader, DEVICE, EVAL_RUNS_PER_SEED, centroids)
...
if all_results_list:
    ...
    for prior_target in unique_priors:
        ...
        name_parts = ["Gaussian_SingleMNIST", prior_target]
