...
parser.add_argument('--prior', type=str, default='GaussianPrior', help='Prior name')
...
prior = GaussianPrior(device=device)
...
print("Initializing prior means using the untrained model...")
model.eval()                                            
...
with torch.no_grad():                                         
    for batch_X, batch_y in train_loader:
        ...
        if class_sums is None:
            class_sums = torch.zeros((num_classes, z_flat.size(1)), device=device)

        for c in range(num_classes):
            mask = (batch_y == c)
            if mask.any():
                class_sums[c] += z_flat[mask].sum(dim=0)
                class_counts[c] += mask.sum()

initial_means = class_sums / class_counts.unsqueeze(1).clamp(min=1)

if isinstance(prior.means, torch.nn.Parameter):
    prior.means.data.copy_(initial_means)
else:
    prior.means = initial_means
...
for epoch in range(num_epochs):
    ...
    epoch_means = class_sums / class_counts.unsqueeze(1).clamp(min=1)
    prior.means = epoch_means.detach()
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'means': prior.means,
            'epoch': epoch + 1
        }, f'../experiments/models/Gaussian/best_loss_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.pth')
    ...
    if val_acc > best_acc:
        ...
        torch.save({
            ...
            'means': prior.means,
            'epoch': epoch + 1
        }, f'../experiments/models/Gaussian/best_acc_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.pth')
