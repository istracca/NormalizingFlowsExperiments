...
def get_pending_experiments():
    ...
    all_combos = list(itertools.product(SCALES, MODELS, PRIORS, OPTIMIZERS, TRANSFORMS, DROPOUT_P, SEEDS))
    pending = []
    
    for combo in all_combos:
        scale, model, prior, opt, trans, dropout, seed = combo
        path = f"../experiments_seed/logs/GMM/GMM_{scale}_{model}_{prior}_{opt}_{trans}_{dropout}_{seed}.log"
...
def run_worker(worker_id, gpu_id, experiments):
    ...
    for i, (scale, model, prior, opt, trans, dropout, seed) in enumerate(experiments):
        print(f"\n[GPU {gpu_id} - Experiment {i+1}/{len(experiments)}]")
        print(f"Params: Scale={scale}, Model={model}, Prior={prior}, Opt={opt}, Transform={trans}, Dropout={dropout}, Seed={seed}")
