    ...
    for combo in base_combos:
        ...
        if prior == 'CheckerboardIB_style':
            for style_variance in STYLE_VARIANCE:
                path = f"../experiments_seeds/models/IB/final_{scale}_{beta}_{model}_{prior}_{opt}_{trans}_{dropout}_{fix_m}_{epochs_warmup}_{style_variance}_{seed}.pth"
                if not os.path.exists(path):
                    pending.append((*combo, style_variance))
                else:
                    print(f"Skipping existing model: {path}")
        else:
            path = f"../experiments_seed/models/IB/final_{scale}_{beta}_{model}_{prior}_{opt}_{trans}_{dropout}_{fix_m}_{epochs_warmup}_{seed}.pth"
