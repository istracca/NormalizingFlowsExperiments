...
class ChannelCouplingLayer(nn.Module):
    def __init__(self, in_channels, cond_dim, hidden_dim=64, dropout_p=0.0):
        ...
        # Use the new conditioned network
    ...
    def forward(self, x, c):
        ...
        # Pass the condition c to the subnetwork
        out = self.net(x_p, c)
...
class ConditionedLinearNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, out_dim, cond_dim, dropout_p=0.0):
        ...
        self.cond_proj1 = nn.Linear(cond_dim, hidden_dim * 2) # Output 2x for FiLM
        self.act1 = nn.LeakyReLU(0.1)
        self.drop1 = nn.Dropout(p=dropout_p)

        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.cond_proj2 = nn.Linear(cond_dim, hidden_dim * 2) # Output 2x for FiLM
...
class SimpleFlow(nn.Module):
    def __init__(self, cond_dim, input_dim=784, hidden_dim=1024, num_layers=8, dropout_p=0.0):
        super().__init__()
        ...
        for i in range(num_layers):
            ...
            # Use the newly created conditioned linear network
...
class GeneralFlow(nn.Module):
    def __init__(self, num_classes=10, cond_dim=64, dropout_p=0.0):
        super().__init__()
        ...
        # --- Linear ---
        self.linear_flow = SimpleFlow(cond_dim=cond_dim, input_dim=784, hidden_dim=2048, num_layers=4, dropout_p=dropout_p)

    def forward(self, x, y):
        # y is the one-hot class vector: (B, num_classes)
        c = self.cond_mlp(y)
        ...
        # --- LINEAR ---
        x = x.view(x.size(0), -1)
        z, log_det_linear = self.linear_flow(x, c) # Pass c
