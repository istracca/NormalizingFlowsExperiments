...
def save_samples_double_colored_ind(model, prior, device, num_attr, epoch, save_dir, temp=0.0):
    """
    Generates sample grids.
    - If num_attr == 2: Generates one 10x10 grid (Digits).
    - If num_attr == 4: Generates TWO grids:
            ...
            for i in range(num_rows):
                for j in range(num_cols):
                    ...
                    for k in range(num_attr):
                        if k == row_attr_idx:
                            # Use the loop variable 'i' for the row attribute
                            part = torch.ones(prior.dims_per_attr) * prior.means[k][i]
                        elif k == col_attr_idx:
                            # Use the loop variable 'j' for the col attribute
                            part = torch.ones(prior.dims_per_attr) * prior.means[k][j]
                        else:
                            # Use the fixed default for other attributes
                            # fixed_defaults[k] should be an integer index (e.g., 0)
                            idx = fixed_defaults[k]
                            part = torch.ones(prior.dims_per_attr) * prior.means[k][idx]
...
def save_samples_double_colored_style(model, prior, device, num_attr, epoch, save_dir, temp=0.0):
    """
    Generates sample grids.
    - If num_attr == 2: Generates one 10x10 grid (Digits).
    - If num_attr == 4: Generates TWO grids:
            ...
            for i in range(num_rows):
                for j in range(num_cols):
                    ...
                    for k in range(num_attr):
                        ...
                        z_parts.append(part)
                    z_parts.append(torch.zeros(1, prior.dims_per_attr).to(device))  # Add zero mean for the last part
                    # 2. Combine and Sample
                    z = prior.get_full_latent(z_parts)
