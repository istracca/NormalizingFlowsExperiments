...
if PRIOR == 'CheckerboardGMM':
    prior = CheckerboardGMM(total_dim=1568, arr_num_classes=[10,10], device=device, scale=SCALE, fixed_means=False).to(device)
elif PRIOR == 'SimpleSplitGMM':
    prior = SimpleSplitGMM(total_dim=1568, arr_num_classes=[10,10], device=device, scale=SCALE, fixed_means=False).to(device)
