...
combinations = list(itertools.product(SCALES, MODELS, PRIORS, OPTIMIZERS, TRANSFORMS, DROPOUT_P))
...
def run_worker(worker_id, gpu_id, experiments):
    ...
    for i, (scale, model, prior, opt, trans, dropout) in enumerate(experiments):
        print(f"\n[GPU {gpu_id} - Experiment {i+1}/{len(experiments)}]")
        print(f"Params: Scale={scale}, Model={model}, Prior={prior}, Opt={opt}, Transform={trans}, Dropout={dropout}")

        cmd = [
            sys.executable, "train_gmm_double.py",
            "--scale", str(scale),
            "--model", model,
            "--optimizer", opt,
            "--prior", prior,
