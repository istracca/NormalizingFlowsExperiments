...
from save_samples import save_samples_double, save_samples_double_style
...
parser.add_argument('--style_variance', type=float, default=1.0, help='Variance of the style component in the CheckerboardIB_style prior')
...
if PRIOR == 'CheckerboardIB':
    prior = CheckerboardIB(total_dim=1568, arr_num_classes=arr_num_classes, beta=BETA, device=device, scale=SCALE, fixed_means=FIXED_MEANS).to(device)
    save_samples_fun = save_samples_double
elif PRIOR == 'SimpleSplitIB':
    prior = SimpleSplitIB(total_dim=1568, arr_num_classes=arr_num_classes, beta=BETA, device=device, scale=SCALE, fixed_means=FIXED_MEANS).to(device)
    save_samples_fun = save_samples_double_style
...
os.makedirs(os.path.dirname(f'../experiments_seed/logs/IB/IB_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.log'), exist_ok=True)
logging.basicConfig(
    filename=f'../experiments_seed/logs/IB/IB_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.log',
    filemode='w',
...
os.makedirs(os.path.dirname(f'../experiments_seed/csv/IB/IB_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.csv'), exist_ok=True)
csv_path = f'../experiments_seed/csv/IB/IB_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.csv'
...
if not os.path.exists(csv_path):
    with open(csv_path, 'w', newline='') as f:
        ...
        writer.writeheader()
save_dir = f'../experiments_seed/samples/IB/IB_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}'
...
for epoch in range(num_epochs):
    ...
    os.makedirs(os.path.dirname(f'../experiments_seed/models/IB/best_loss_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.pth'), exist_ok=True)
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'means': prior.means,
            'epoch': epoch + 1
        }, f'../experiments_seed/models/IB/best_loss_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.pth')
    ...
    if val_acc > best_acc:
        ...
        torch.save({
            ...
            'means': prior.means,
            'epoch': epoch + 1
        }, f'../experiments_seed/models/IB/best_acc_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.pth')
...
torch.save({
    ...
    'optimizer_state_dict': optimizer.state_dict(),
    'means': prior.means
}, f'../experiments_seed/models/IB/final_{SCALE}_{MODEL}_{PRIOR}_{BETA}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}_{FIXED_MEANS}_{EPOCHS_WARMUP}_{STYLE_VARIANCE}_{SEED}.pth')