...
os.makedirs(os.path.dirname(f'../experiments/logs/Conditional/Conditional_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.log'), exist_ok=True)
logging.basicConfig(
...
os.makedirs(os.path.dirname(f'../experiments/csv/Conditional/Conditional_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.csv'), exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    base_save_path = f'../experiments/models/Conditional/'
    ...
    if epoch % 10 == 0:
        save_samples_double_conditional(model, prior, device, epoch, arr_num_classes=arr_num_classes, save_dir=save_dir + f'/epoch_{epoch}', temp=0)
