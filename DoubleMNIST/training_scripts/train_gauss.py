...
from save_samples import save_samples_double_gaussian
...
os.makedirs('../experiments/models/Gaussian', exist_ok=True)
os.makedirs('../experiments/logs/Gaussian', exist_ok=True)
os.makedirs('../experiments/csv/Gaussian', exist_ok=True)
save_dir = f'../experiments/samples/Gaussian/Gaussian_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}'
...
os.makedirs(os.path.dirname(f'../experiments/logs/Gaussian/Gaussian_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.log'), exist_ok=True)
logging.basicConfig(
    filename=f'../experiments/logs/Gaussian/Gaussian_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.log',
    filemode='w',
...
os.makedirs(os.path.dirname(f'../experiments/csv/Gaussian/Gaussian_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.csv'), exist_ok=True)
csv_path = f'../experiments/csv/Gaussian/Gaussian_{MODEL}_{PRIOR}_{OPTIMIZER}_{TRANSFORM}_{DROPOUT}.csv'
if os.path.exists(csv_path):
    os.remove(csv_path)

headers = ['epoch', 'train_loss', 'val_loss', 'train_acc_ind', 'train_acc_comb', 'val_acc_ind', 'val_acc_comb', 'lr']
...
print("Initializing independent and combinatorial prior means...")
model.eval()
...
with torch.no_grad():
    for batch_X, batch_y in train_loader:
        ...
        for attr_idx in range(num_attributes):
            for c in range(arr_num_classes[attr_idx]):
                mask = (batch_y[:, attr_idx] == c)
                if mask.any():
                    indep_sums[attr_idx][c] += z_flat[mask].sum(dim=0)
                    indep_counts[attr_idx][c] += mask.sum()

        flat_y = (batch_y * strides).sum(dim=1)
        for c in range(total_joint_classes):
            mask = (flat_y == c)
            if mask.any():
                comb_sums[c] += z_flat[mask].sum(dim=0)
                comb_counts[c] += mask.sum()
...
for epoch in range(num_epochs):
    ...
    for batch_X, batch_y in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        with torch.no_grad():                                                
            preds_ind, _, preds_comb, _ = prior.classify(z_flat)
            
            matched_ind = (preds_ind == batch_y).all(dim=1)
            matched_comb = (preds_comb == flat_y)

            train_correct_ind += matched_ind.sum().item()
            train_correct_comb += matched_comb.sum().item()
            train_total += batch_y.size(0)

    train_loss /= len(train_loader)
    train_acc_ind = train_correct_ind / train_total
    train_acc_comb = train_correct_comb / train_total
    ...
    with torch.no_grad():
        for batch_X, batch_y in val_loader:
            ...
            val_correct_ind += matched_ind.sum().item()
            val_correct_comb += matched_comb.sum().item()
            val_total += batch_y.size(0)

    val_loss /= len(val_loader)
    val_acc_ind = val_correct_ind / val_total
    val_acc_comb = val_correct_comb / val_total
    ...
    base_save_path = f'../experiments/models/Gaussian/'
    ...
    save_dict = {
        ...
        'independent_means': prior.independent_means,
        'combinatorial_means': prior.combinatorial_means,
        'epoch': epoch + 1
    }
    ...
    if val_acc_ind > best_acc_ind:
        best_acc_ind = val_acc_ind
        torch.save(save_dict, os.path.join(base_save_path, f'best_acc_ind_{suffix}'))

    if val_acc_comb > best_acc_comb:
        best_acc_comb = val_acc_comb
        torch.save(save_dict, os.path.join(base_save_path, f'best_acc_comb_{suffix}'))

    logger.info(
        f'Epoch {epoch+1}/{num_epochs} | '
        f'Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f} | '
        f'Train Acc (Ind/Comb): {train_acc_ind:.4f}/{train_acc_comb:.4f} | '
        f'Val Acc (Ind/Comb): {val_acc_ind:.4f}/{val_acc_comb:.4f}'
    )
    ...
    with open(csv_path, 'a', newline='') as f:
        ...
        writer.writerow({
            ...
            'val_loss': val_loss,
            'train_acc_ind': train_acc_ind,
            'train_acc_comb': train_acc_comb,
            'val_acc_ind': val_acc_ind,
            'val_acc_comb': val_acc_comb,
            'lr': current_lr
        })

    if epoch % 10 == 0:
        save_samples_double_gaussian(model, prior, device, epoch, save_dir=save_dir, mode='independent', temp=0)
        save_samples_double_gaussian(model, prior, device, epoch, save_dir=save_dir, mode='combinatorial', temp=0)
