...
DATASET_FILE = 'balanced_double_mnist.npz'
...
print(f"Loading Data: {DATASET_FILE}...")
data = np.load(f'../data/{DATASET_FILE}')
...
for config in combinations:
    ...
    if is_ib:
        config_id = f"{config['TYPE']}_{config['SCALE']}_{config['MODEL']}_{prior_name}_{config['BETA']}_{config['OPTIMIZER']}_{config['TRANSFORM']}_{config['DROPOUT']}_{config['FIXED_MEANS']}_{config['EPOCHS_WARMUP']}_{config['STYLE_VARIANCE']}"
    else:
    ...
    search_pattern = f"../experiments_seed/models/{folder_type}/{config_id.rstrip('_')}_*.pth"
    ...
    for model_path in all_model_paths:
        try:
            ...
            prior_args = {
                'total_dim': TOTAL_DIM, 'arr_num_classes': ARR_NUM_CLASSES,
        ...
        v_acc_all, v_acc_attr, v_losses, v_gen, v_cls = evaluate_model(model, prior, val_loader, DEVICE, EVAL_RUNS_PER_SEED)
        ...
        t_acc_all, t_acc_attr, t_losses, t_gen, t_cls = evaluate_model(model, prior, test_loader, DEVICE, EVAL_RUNS_PER_SEED)
        ...
        for param in PARAMS_TO_TRACK:
            if param in df_prior.columns:
                ...
                if len(unique_vals) == 1:
                    name_parts.append(f"{param.lower()}_{unique_vals[0]}")
