...
class SimpleFlow(nn.Module):
    def __init__(self, input_dim=1568, hidden_dim=1024, num_layers=8, dropout_p=0.0):
        super().__init__()
...
class GeneralFlow(nn.Module):
    def __init__(self, dropout_p=0.0):
        ...
        for _ in range(8):
            ...
            self.flow1_inv1x1.append(Invertible1x1Conv(channels=12))
            self.flow1_couplings.append(ChannelCouplingLayer(in_channels=12, hidden_dim=64, dropout_p=dropout_p))
        
        # --- SCALE 2: 14x28x12 -> 7x14x48 ---
        ...
        for _ in range(8):
            # Channels are 48 now
            self.flow2_inv1x1.append(Invertible1x1Conv(channels=48))
            self.flow2_couplings.append(ChannelCouplingLayer(in_channels=48, hidden_dim=128, dropout_p=dropout_p))
        
        # --- GLOBAL: Linear ---
        # Flatten input: 48 * 7 * 14 = 4704
        self.linear_flow = SimpleFlow(input_dim=4704, hidden_dim=2048, num_layers=4, dropout_p=dropout_p)
    def forward(self, x):
        # Ensure 4D input
        if x.dim() == 2: x = x.view(-1, 3, 28, 56)
