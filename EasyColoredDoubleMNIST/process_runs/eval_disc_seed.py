...
for config in combinations:
    ...
    original_model_path = f"../experiments/models/Disc/{version}/{config_id}.pth"
    if os.path.exists(original_model_path):
        all_model_paths.append(original_model_path)
    
    search_pattern = f"../experiments_seed/models/Disc/{version}/{config_id.rstrip('_')}_*.pth"
    ...
    for model_path in all_model_paths:
        try:
            ...
            if filename.startswith(config_id + "_"):
                seed_str = filename[len(config_id)+1:]
                try:
    ...
    for model_target in unique_models:
        ...
        for param in PARAMS_TO_TRACK:
            if param in df_model.columns:
                unique_vals = df_model[param].dropna().unique()
