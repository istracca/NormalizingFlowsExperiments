...
def get_pending_experiments():
    ...
    base_combos = list(itertools.product(MODELS, OPTIMIZERS, TRANSFORMS, DROPOUT_P, VERSION, SEEDS))
    pending = []
    
    for combo in base_combos:
        model, opt, trans, dropout, version, seed = combo
        
        path = f"../experiments_seeds/logs/Disc/{version}/Disc_{model}_{opt}_{trans}_{dropout}_{seed}.log"
...
def run_worker(worker_id, gpu_id, experiments):
    ...
    for i, (model, opt, trans, dropout, version, seed) in enumerate(experiments):
        print(f"\n[GPU {gpu_id} - Experiment {i+1}/{len(experiments)}]")
        print(f"Params: Model={model}, Opt={opt}, Transform={trans}, Dropout={dropout}, Version={version}, Seed={seed}")

        cmd = [
            sys.executable, "train_disc_seed.py",
            "--model", model,
            "--optimizer", opt,
            "--transform", str(trans),
            "--dropout", str(dropout),
