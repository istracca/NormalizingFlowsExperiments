...
def get_pending_experiments():
    ...
    base_combos = list(itertools.product(SCALES, MODELS, PRIORS, OPTIMIZERS, BETAS, TRANSFORMS, DROPOUT_P, VERSION, FIXED_MEANS, EPOCHS_WARMUP, SEEDS))
    pending = []
    
    for combo in base_combos:
        scale, model, prior, opt, beta, trans, dropout, version, fix_m, epochs_warmup, seed = combo
        
        if prior == 'CheckerboardIB_style':
            for style_variance in STYLE_VARIANCE:
                path = f"../experiments_seeds/logs/IB/{version}/IB_{scale}_{model}_{prior}_{beta}_{opt}_{trans}_{dropout}_{fix_m}_{epochs_warmup}_{style_variance}_{seed}.log"
                if not os.path.exists(path):
                    pending.append((*combo, style_variance))
                else:
                    print(f"Skipping existing log: {path}")
        else:
            path = f"../experiments_seeds/logs/IB/{version}/IB_{scale}_{model}_{prior}_{beta}_{opt}_{trans}_{dropout}_{fix_m}_{epochs_warmup}_{seed}.log"
            if not os.path.exists(path):
...
def run_worker(worker_id, gpu_id, experiments):
    ...
    for i, (scale, model, prior, opt, beta, trans, dropout, version, fix_m, epochs_warmup, seed, style_variance) in enumerate(experiments):
        print(f"\n[GPU {gpu_id} - Experiment {i+1}/{len(experiments)}]")
        print(f"Params: Scale={scale}, Model={model}, Prior={prior}, Beta={beta}, Opt={opt}, Transform={trans}, Dropout={dropout}, Version={version}, FixedMeans={fix_m}, EpochsWarmup={epochs_warmup}, StyleVariance={style_variance}, Seed={seed}")

        cmd = [
            sys.executable, "train_ib_seed.py",
            "--scale", str(scale),
            "--model", model,
            "--optimizer", opt,
            "--prior", prior,
