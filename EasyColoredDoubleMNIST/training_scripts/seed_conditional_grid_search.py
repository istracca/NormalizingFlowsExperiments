...
def get_pending_experiments():
    ...
    base_combos = list(itertools.product(MODELS, PRIORS, OPTIMIZERS, TRANSFORMS, DROPOUT_P, VERSION, COND_DIMS, SEEDS))
    pending = []
    
    for combo in base_combos:
        model, prior, opt, trans, dropout, version, cond_dim, seed = combo
        
        path = f"../experiments_seeds/logs/Conditional/{version}/Conditional_{model}_{prior}_{opt}_{trans}_{dropout}_{seed}.log"
...
def run_worker(worker_id, gpu_id, experiments):
    ...
    for i, (model, prior, opt, trans, dropout, version, cond_dim, seed) in enumerate(experiments):
        print(f"\n[GPU {gpu_id} - Experiment {i+1}/{len(experiments)}]")
        print(f"Params: Model={model}, Prior={prior}, Opt={opt}, Transform={trans}, Dropout={dropout}, Version={version}, CondDim={cond_dim}, Seed={seed}")
