...
MODEL_CONFIG["PATH"] = (
    f"../experiments/models/IB/2_attr/best_loss_{SCALE}_{MODEL_CONFIG['MODEL']}_{MODEL_CONFIG['PRIOR']}_"
    f"{BETA}_{MODEL_CONFIG['OPTIMIZER']}_{MODEL_CONFIG['TRANSFORM']}_{MODEL_CONFIG['DROPOUT']}_"
    f"{MODEL_CONFIG['FIXED_MEANS']}_{MODEL_CONFIG['EPOCHS_WARMUP']}_{MODEL_CONFIG['STYLE_VARIANCE']}.pth"
)
...
PLOT_DIR = f"plots/IB_Intervention_Grids_Beta_{BETA}/"
...
def find_and_plot_progressions(model, prior, disc_model, loader, device):
    ...
    for grp_name, grp_title in [("1_interv", "1 Intervention"), ("2_interv", "2 Interventions")]:
        plot_progression_grid(final_plots[grp_name]["style"], f"Style vs Lambda ({grp_title})", os.path.join(PLOT_DIR, f"plot_1_style_{grp_name}_IB_{BETA}.png"), "style")
        plot_progression_grid(final_plots[grp_name]["cc"], f"Cyclic Consistency vs Lambda ({grp_title})", os.path.join(PLOT_DIR, f"plot_2_cc_{grp_name}_IB_{BETA}.png"), "cc")
        plot_progression_grid(final_plots[grp_name]["disc"], f"Disc Accuracy vs Lambda ({grp_title})", os.path.join(PLOT_DIR, f"plot_3_disc_{grp_name}_IB_{BETA}.png"), "disc")
    print("Done!")
...
if __name__ == "__main__":
    ...
    prior = prior_class(total_dim=4704, arr_num_classes=[10, 10], beta=BETA, device=DEVICE, scale=SCALE, fixed_means=MODEL_CONFIG['FIXED_MEANS'])
