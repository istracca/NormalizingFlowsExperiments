...
def collect_cnf_samples(loader, disc_model):
    print("Collecting CNF (Conditional) Samples...")
    pool = []
    ...
    with torch.no_grad():
        for batch_X, _ in loader:
            ...
            for int_type in ['attr0', 'attr1', 'both']:
                ...
                succ = (preds_post[:, 0] == t0) & (preds_post[:, 1] == t1) & (int_c_left == orig_c_left) & (int_c_right == orig_c_right)
...
def collect_hce_samples(loader, disc_model, train_loader):
    print("Collecting HCE (Gaussian) Samples...")
    pool_c, pool_i = [], []
        ...
        for batch_X, _ in loader:
            ...
            for int_type in ['attr0', 'attr1', 'both']:
                ...
                for idx in torch.nonzero(succ_i, as_tuple=False).squeeze(-1):
                    pool_i.append({
                        'img_orig': batch_X[idx].cpu().numpy(), 'img_interv': img_int_i[idx].cpu().numpy(),
                ...
                for idx in torch.nonzero(succ_c, as_tuple=False).squeeze(-1):
                    pool_c.append({
                        'img_orig': batch_X[idx].cpu().numpy(), 'img_interv': img_int_c[idx].cpu().numpy(),
                        'orig_classes': [preds_pre[idx, 0].item(), preds_pre[idx, 1].item()], 'tgt_classes': [t0[idx].item(), t1[idx].item()], 'int_type': int_type
                    })
...
def collect_pmf_samples(loader, disc_model, model_type="GMM"):
    print(f"Collecting PMF ({model_type}) Samples...")
    pool = []
    gen_module = importlib.import_module("hybrid_v3_1x1_double")
            ...
            for batch_X, _ in loader:
                ...
                for int_type in ['attr0', 'attr1', 'both']:
                    ...
                    for lam in LAMBDAS_PMF:
                        ...
                        succ = (preds_post_prior[:, 0] == t0) & (preds_post_prior[:, 1] == t1) & (preds_post_disc[:, 0] == t0) & (preds_post_disc[:, 1] == t1) & (c_left == orig_c_left) & (c_right == orig_c_right)
                        for idx in torch.nonzero(succ, as_tuple=False).squeeze(-1):
                            pool.append({
...
def plot_comprehensive_interventions(data, save_path="plots/comprehensive_interventions_successes.png"):
    ...
    populate_axes(pmf_subfigs[0].subplots(2, 5, gridspec_kw={'hspace': 0.0}), data['GMM'], 'GMM')
    
    pmf_subfigs[1].suptitle("IB", fontsize=28)
    populate_axes(pmf_subfigs[1].subplots(2, 5, gridspec_kw={'hspace': 0.0}), data['IB'], 'IB')
...
if __name__ == "__main__":
    ...
    X_test_tensor = torch.tensor(data_np['X_test'].transpose(0, 3, 1, 2), dtype=torch.float32)[:2048]
    y_test_tensor = torch.tensor(data_np['y_test'][:, 0:2], dtype=torch.long)[:2048]
    ...
    final_data = {
        'GMM': filter_balanced_samples(pool_gmm, n_samples=10, param_key='scale'),
        'IB': filter_balanced_samples(pool_ib, n_samples=10, param_key='beta'),
