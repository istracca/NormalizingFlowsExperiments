...
def collect_cnf_samples(loader, disc_model):
    print("Collecting CNF (Conditional) Samples (CC Success, Style Failure)...")
    pool = []
    ...
    with torch.no_grad():
        for batch_X, _ in loader:
            ...
            for int_type in ['attr0', 'attr1', 'both']:
                ...
                cc_succ = (preds_post[:, 0] == t0) & (preds_post[:, 1] == t1)
                style_fail = (int_c_left != orig_c_left) | (int_c_right != orig_c_right)
                succ = cc_succ & style_fail
...
def collect_hce_samples(loader, disc_model, train_loader):
    print("Collecting HCE (Gaussian) Samples (CC Success, Style Failure)...")
    pool_c, pool_i = [], []
...
def collect_pmf_samples(loader, disc_model, model_type="GMM"):
    print(f"Collecting PMF ({model_type}) Samples (CC Success, Style Failure)...")
    pool = []
    gen_module = importlib.import_module("hybrid_v3_1x1_double")
...
def plot_comprehensive_interventions(data, save_path="plots/comprehensive_interventions_no_style.png"):
