...
parser = argparse.ArgumentParser(description='Evaluate Cyclic Consistency and Style on Colored MNIST with IB Prior.')
...
base_filename_no_ext = (
    f"best_loss_{SCALE}_{MODEL_CONFIG['MODEL']}_{MODEL_CONFIG['PRIOR']}_"
    f"{BETA}_{MODEL_CONFIG['OPTIMIZER']}_{MODEL_CONFIG['TRANSFORM']}_{MODEL_CONFIG['DROPOUT']}_"
    f"{MODEL_CONFIG['FIXED_MEANS']}_{MODEL_CONFIG['EPOCHS_WARMUP']}_{MODEL_CONFIG['STYLE_VARIANCE']}"
)
...
for seed in SEEDS:
    if seed == "original":
        path = f"../experiments/models/IB/2_attr/{base_filename_no_ext}.pth"
        MODELS_TO_LOAD["Original"] = path
    else:
        path = f"../experiments_seed/models/IB/2_attr/{base_filename_no_ext}_{seed}.pth"
...
RESULTS_CSV = f"csv/IB_Beta_{BETA}/cyc_cons_IB_aggregate.csv"
...
def evaluate_single_model(model, prior, disc_model, loader, device, model_name):
    ...
    for lam in LAMBDAS:
        for int_type in ["attr0", "attr1", "both"]:
            ...
            results.append({
                'Model': model_name,
                'lambda': lam, 'intervention_group': int_type, 'concepts_intervened': num_interventions,
