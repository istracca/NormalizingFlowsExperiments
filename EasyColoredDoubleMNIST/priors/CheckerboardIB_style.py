import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np

class CheckerboardIB(nn.Module):
    def __init__(self, total_dim, arr_num_classes, beta, device, scale, fixed_means=False):
        super().__init__()

        self.num_attr = len(arr_num_classes) + 1
        self.dims_per_attr = total_dim // self.num_attr
        self.remainder = total_dim % self.num_attr
        self.beta = beta

        if fixed_means:
            self.means = []
            for i in range(self.num_attr - 1):
                mean = torch.randn(arr_num_classes[i], self.dims_per_attr + (1 if i < self.remainder else 0), device=device) * scale
                self.means.append(mean)
        else:
            self.means = nn.ParameterList()
            for i in range(self.num_attr - 1):
                mean = nn.Parameter(torch.randn(arr_num_classes[i], self.dims_per_attr + (1 if i < self.remainder else 0), device=device) * scale)
                mean.requires_grad = True
                self.means.append(mean)
            
 
    def get_loss(self, z, sldj, labels):
        y = labels
        z_flat = z.view(z.shape[0], -1)

        log_pz = 0
        loss_cls = 0

        for i in range(self.num_attr - 1):

            dists = -0.5 * ((z_flat[:, i::self.num_attr].unsqueeze(1) - self.means[i].unsqueeze(0))**2).sum(2)

            log_pz += torch.logsumexp(dists, dim=1)
            loss_cls += F.cross_entropy(dists, y[:, i])

        dist_style = -0.5 * (z_flat[:, (self.num_attr - 1)::self.num_attr] ** 2).sum(dim=1)
        log_pz += dist_style

        loss_cls /= (self.num_attr - 1)
        loss_gen = - (log_pz + sldj).mean() / z_flat.shape[1]
        
        return loss_gen + self.beta * loss_cls, loss_gen, loss_cls


    def classify(self, z_flat):
        chunks = [z_flat[:, i::self.num_attr] for i in range(self.num_attr)]
        preds = []
        complete_logits = []

        for i in range(self.num_attr - 1):
            d = -((chunks[i].unsqueeze(1) - self.means[i].unsqueeze(0))**2).sum(2)
            
            preds.append(d.argmax(1))
            complete_logits.append(d)

        # Stack along dimension 1 to get shape (batch_size, num_attr)
        preds = torch.stack(preds, dim=1)
        return preds, complete_logits
    

    def get_full_latent(self, z_list):
        """
        z_list: list of vectors, one per attribute
        """
        z_flat = torch.zeros(z_list[0].size(0), self.num_attr * self.dims_per_attr + self.remainder, device=z_list[0].device)
        for i in range(self.num_attr):
            z_flat[:, i::self.num_attr] = z_list[i]
        return z_flat
    
    def get_parts(self, z_flat):
        parts = []
        for i in range(self.num_attr):
            parts.append(z_flat[:, i::self.num_attr])
        return parts