...
class CheckerboardGMM(nn.Module):
    def __init__(self, total_dim, arr_num_classes, device, scale, fixed_means=True):
        ...
        if fixed_means:
            ...
            for i in range(self.num_attr - 1):
                mean = torch.randn(arr_num_classes[i], dims_per_attr + (1 if i < self.remainder else 0), device=device) * scale
                self.means.append(mean)
        else:
            self.means = nn.ParameterList()
            for i in range(self.num_attr - 1):
                mean = nn.Parameter(torch.randn(arr_num_classes[i], dims_per_attr + (1 if i < self.remainder else 0), device=device) * scale)
    ...
    def get_loss(self, z, sldj, labels):
        ...
        nll += 0.5 * (z_flat[:, (self.num_attr - 1)::self.num_attr] ** 2).sum(dim=1) + 0.5 * self.dims_per_attr * np.log(2 * np.pi) # Prior for the last part (standard normal)
