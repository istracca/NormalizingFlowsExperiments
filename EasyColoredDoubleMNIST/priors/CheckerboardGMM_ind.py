...
class CheckerboardGMM(nn.Module):
    def __init__(self, total_dim, arr_num_classes, device, scale, fixed_means=True):
        ...
        if fixed_means:
            for i in range(self.num_attr):
                mean = torch.tensor([j * (-1)**j for j in range(arr_num_classes[i])], dtype=torch.float32, device=device).unsqueeze(1) * scale
                self.means.append(mean)
            
        else:
            for i in range(self.num_attr):
                mean = nn.Parameter(torch.tensor([j * (-1)**j for j in range(arr_num_classes[i])], dtype=torch.float32, device=device).unsqueeze(1) * scale)
    ...
    def get_loss(self, z, sldj, labels):
        ...
        for i in range(self.num_attr):
            ...
            diff = z_subset - torch.ones(z_subset.shape[0], self.dims_per_attr, device=z_subset.device) * self.means[i][y[:, i]]
    ...
    def classify(self, z_flat):
        ...
        for i in range(self.num_attr):
            ...
            sq_dist = -((z_subset.unsqueeze(1) - torch.ones(z_subset.shape[0], self.means[i].shape[0], self.dims_per_attr, device=z_subset.device) * self.means[i].unsqueeze(0))**2).sum(2)
