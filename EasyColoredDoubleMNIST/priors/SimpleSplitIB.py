...
class SimpleSplitIB(nn.Module):
    ...
    def get_loss(self, z, sldj, labels):
        ...
        for i in range(self.num_attr):
            ...
            end = (i + 1) * self.dims_per_attr + min(i + 1, self.remainder)
            dists = -0.5 * ((z_flat[:, start:end].unsqueeze(1) - self.means[i].unsqueeze(0))**2).sum(2)
    ...
    def classify(self, z_flat):
        chunks = [z_flat[:, i * self.dims_per_attr : (i + 1) * self.dims_per_attr + (1 if i < self.remainder else 0)] for i in range(self.num_attr)]
        preds = []
        complete_logits = []
        for i in range(self.num_attr):
